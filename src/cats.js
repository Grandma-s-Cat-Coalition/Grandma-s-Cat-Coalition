...
export function buildCatBio(cat) {
  ...
  if (firstPersonNotes) return `${sourceDescription}${waitingSentence ? `${waitingSentence}` : ''}`;
  const descriptionSentence = sourceDescription ? ` I’m ${sourceDescription.charAt(0).toLowerCase()}${sourceDescription.slice(1).replace(/\bloves\b/gi, 'love').replace(/\bis\b/gi, 'am')}.` : '';
